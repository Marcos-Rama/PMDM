...
public class T015BottomNavViewRespondsToClicks {
    ...
    public void checkToast1IsShown() {
        ...
        Espresso.onView(withText("Has clicado en Digimon 1")).inRoot(isOverlayWindow()).check(matches(isDisplayed()));
    }

    @Test
    public void checkToast2IsShown() {
        Espresso.onView(withBottomMenuItem("item2")).perform(click());
        Espresso.onView(withText("Has clicado en Digimon 2")).inRoot(isOverlayWindow()).check(matches(isDisplayed()));
    }

    @Test
    public void checkToast3IsShown() {
        Espresso.onView(withBottomMenuItem("item3")).perform(click());
        Espresso.onView(withText("Clicaste Digimon 3")).inRoot(isOverlayWindow()).check(matches(isDisplayed()));
    }
