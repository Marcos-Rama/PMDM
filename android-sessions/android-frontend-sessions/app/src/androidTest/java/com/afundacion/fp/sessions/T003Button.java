...
public class T003Button {
    ...
    public void checkRegisterButtonIsCorrect() {
        Matcher<View> parent = withChild(withText("Registro"));
        Espresso.onView(withText("Registro")).check(matches(withClassName(containsString("Button"))));
        Espresso.onView(withText("Registro")).check(isBottomAlignedWith(parent));
        Espresso.onView(withText("Registro")).check(isLeftAlignedWith(parent));
        Espresso.onView(withText("Registro")).check(isRightAlignedWith(parent));
        Espresso.onView(withText("Registro")).check(hasDeclaredId());
    }
    ...
    public void checkRegisterActivityWorksProperly() throws ClassNotFoundException, NoSuchFieldException {
        ...
        Espresso.onView(editTextWithHint("jane.doe")).perform(typeText(randomText));
