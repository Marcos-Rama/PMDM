...
public class T006ActivityForLogin extends TestRun {
    ...
    public static void setUp() throws ClassNotFoundException, IOException, InterruptedException {
        ...
        Server.name = Server.name + "/testing/Dev24";
    ...
    public void checkButtonsAreCorrect() {
        Matcher<View> parent = withChild(withText("Loguearse"));
        Espresso.onView(withText("Loguearse")).check(matches(withClassName(Matchers.containsString("Button"))));
        Espresso.onView(withText("Loguearse")).check(isBottomAlignedWith(parent));
        Espresso.onView(withText("Loguearse")).check(isLeftAlignedWith(parent));
        Espresso.onView(withText("Loguearse")).check(isRightAlignedWith(parent));
        Espresso.onView(withText("Loguearse")).check(hasDeclaredId());
        Espresso.onView(withText("¿Aún no tienes cuenta?")).check(matches(withClassName(Matchers.containsString("Button"))));
        Espresso.onView(withText("¿Aún no tienes cuenta?")).check(isLeftAlignedWith(parent));
        Espresso.onView(withText("¿Aún no tienes cuenta?")).check(isRightAlignedWith(parent));
        Espresso.onView(withText("Loguearse")).check(isTopToBottomOf(withText("¿Aún no tienes cuenta?")));
    }


    @Test
    public void checkGoToRegisterButtonWorksForthAndBack() throws JSONException {
        Espresso.onView(withText("¿Aún no tienes cuenta?")).perform(click());
        String randUser = "Dev24User" + (new Date()).getTime();
        String randPass = "Dev24Pass" + (new Date()).getTime();
        Espresso.onView(withHint("jane.doe")).perform(typeText(randUser));
        Espresso.closeSoftKeyboard();
        Espresso.onView(withHint("Contraseña")).perform(typeText(randPass));
        Espresso.closeSoftKeyboard();
        Espresso.onView(withText("Registro")).perform(click());
        ...
        Espresso.onView(withText("Registro")).check(doesNotExist());
    }
